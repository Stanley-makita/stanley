-- Migration 295: fix desempate ausente na resolução de "comprador principal"
--
-- Sete funções ao vivo resolvem o nome do cliente de um processo via
-- LEFT JOIN LATERAL em processo_compradores, escolhendo o comprador
-- marcado como `principal`:
--   ORDER BY principal DESC NULLS LAST LIMIT 1
-- Quando NENHUM comprador do processo está marcado como principal (caso
-- comum — o campo é opcional), essa ORDER BY não tem critério de
-- desempate: o Postgres pode devolver qualquer um dos compradores
-- empatados, não necessariamente o titular real do contrato.
--
-- Efeito observado: em Financeiro > Análise de Comissões > Financiamento,
-- um processo cujo titular é "Frederico Kiko" aparecia com o nome de um
-- co-comprador não-principal ("Adalto Luiz Barreiros"), enquanto a mesma
-- tela de Negócios > Financiamento mostrava o nome certo — porque essa
-- tela usa client-side `compradores.find(c=>c.principal) ?? compradores[0]`
-- (VisaoTabela.tsx:117), com `compradores` já ordenado por
-- `principal DESC, created_at ASC` (useProcessoCompradores.ts:17-18).
--
-- Fix: adicionar `created_at ASC` (ou `pcomp.created_at ASC`, conforme o
-- alias usado) como segundo critério em toda ORDER BY dessas LATERALs —
-- mesmo desempate que o hook e o VisaoTabela já usam, e que os LATERALs
-- de corretor/imobiliária na migration 293 já tinham (`criado_em ASC`).
-- Nenhuma outra lógica muda nestas 7 funções — apenas esse ORDER BY.
--
-- Funções corrigidas (todas ativas, chamadas pelo frontend hoje):
--   1. puxar_processos_emitidos      (fluxo de Fechamento)
--   2. puxar_contratos               (fluxo de Fechamento)
--   3. emissoes_mes_preview          (aba Emissões)
--   4. contas_a_receber_mes_vivo     (aba A Receber)
--   5. garantir_conta_receber_processo (lançamento de NF/Recebimento)
--   6. analise_comissoes_mes         (Análise de Comissões > Financiamento)
--   7. analise_comissoes_contratos_mes (Análise de Comissões > Contratos)
--
-- `contas_a_receber_mes_preview` (migration 272) tem o mesmo bug mas foi
-- substituída por contas_a_receber_mes_vivo (274) e não é mais chamada
-- pelo frontend — deixada como está, órfã.

-- ============================================================
-- 1. puxar_processos_emitidos (corpo idêntico à migration 236, só o
--    ORDER BY do LATERAL de comprador ganha o desempate)
-- ============================================================
...
-- ============================================================
-- 2. puxar_contratos (corpo idêntico à migration 236)
-- ============================================================
...
-- ============================================================
-- 3. emissoes_mes_preview (corpo idêntico à migration 272)
-- ============================================================
...
--    (corpo idêntico à migration 274)
-- ============================================================
...
-- ============================================================
-- 5. analise_comissoes_mes (corpo idêntico à migration 292)
-- ============================================================
...
-- ============================================================
-- 6. analise_comissoes_contratos_mes (corpo idêntico à migration 293)
-- ============================================================
