-- Atualiza as 3 funções de emissão:
-- 1. Filtra apenas modalidades de financiamento imobiliário + CGI (exclui 'Contrato')
-- 2. Corrige performance_por_banco: GROUP BY p.banco_id em vez de b.nome para evitar
--    colapso incorreto quando bancos têm cor NULL ou nomes idênticos

-- ─── resumo_estoque ──────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS resumo_estoque(UUID);

CREATE FUNCTION resumo_estoque(p_empresa_id UUID)
RETURNS TABLE (
...
DROP FUNCTION IF EXISTS emissoes_por_semana(UUID, INT, INT);

CREATE FUNCTION emissoes_por_semana(
  p_empresa_id UUID,
  p_mes        INT,
  p_ano        INT
)
...
GRANT EXECUTE ON FUNCTION emissoes_por_semana(UUID, INT, INT) TO authenticated;

-- ─── performance_por_banco ───────────────────────────────────────────────────

DROP FUNCTION IF EXISTS performance_por_banco(UUID, INT, INT);

CREATE FUNCTION performance_por_banco(
  p_empresa_id UUID,
  p_mes        INT,
  p_ano        INT
)
...
BEGIN
  ...
  WHERE p.empresa_id = p_empresa_id
    ...
    AND EXTRACT(MONTH FROM p.data_emissao) = p_mes
    AND EXTRACT(YEAR  FROM p.data_emissao) = p_ano
  GROUP BY p.banco_id, b.nome, b.cor
  ORDER BY realizado DESC;
END;
$$;

GRANT EXECUTE ON FUNCTION performance_por_banco(UUID, INT, INT) TO authenticated;
