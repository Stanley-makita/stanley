...
import {
  useTodasImobiliarias,
  useCriarImobiliaria,
  useAtualizarImobiliaria,
  useExcluirImobiliaria,
} from '@/hooks/configuracoes/useParceirosComerciais'
import type { Imobiliaria, TipoImobiliaria } from '@/types/parceiros'
import { TIPO_IMOBILIARIA_LABEL } from '@/types/parceiros'
...
const VAZIO: FormState = { nome: '', tipo: 'imobiliaria', cnpj: '', telefone: '', email: '', observacao: '', ativo: true }

function paraForm(r: Imobiliaria): FormState {
  return {
    nome: r.nome,
    tipo: r.tipo,
    cnpj: r.cnpj ?? '',
...
interface FormProps {
  inicial?: Imobiliaria
  onSalvar: (dados: Partial<Imobiliaria>) => void
  onCancelar: () => void
  isPending: boolean
}

function FormImobiliaria({ inicial, onSalvar, onCancelar, isPending }: FormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      <div className="space-y-1.5">
        ...
        <Input placeholder="Ex: Imobiliária Central" value={form.nome} onChange={(e) => set('nome', e.target.value)} autoFocus />
      </div>

      <div className="space-y-1.5">
        <Label>Tipo</Label>
        <Select value={form.tipo} onValueChange={(v) => set('tipo', v as TipoImobiliaria)}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            {(Object.keys(TIPO_IMOBILIARIA_LABEL) as TipoImobiliaria[]).map((t) => (
              <SelectItem key={t} value={t}>{TIPO_IMOBILIARIA_LABEL[t]}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
...
export function ImobiliariasLista() {
  const { data: itens = [], isLoading } = useTodasImobiliarias()
  const criar = useCriarImobiliaria()
  const atualizar = useAtualizarImobiliaria()
  const excluir = useExcluirImobiliaria()

  const [modalAberto, setModalAberto] = useState(false)
  const [editando, setEditando] = useState<Imobiliaria | undefined>(undefined)
  const [confirmandoExclusao, setConfirmandoExclusao] = useState<string | null>(null)

  function abrirCriar() { setEditando(undefined); setModalAberto(true) }
  function abrirEditar(r: Imobiliaria) { setEditando(r); setModalAberto(true) }
  function fecharModal() { setModalAberto(false); setEditando(undefined) }

  async function handleSalvar(dados: Partial<Imobiliaria>) {
    try {
      if (editando) {
        await atualizar.mutateAsync({ id: editando.id, ...dados })
        toast.success('Imobiliária/construtora atualizada.')
      } else {
        await criar.mutateAsync(dados as Imobiliaria)
        toast.success('Imobiliária/construtora cadastrada.')
      }
      fecharModal()
    } catch {
    ...
    <div className="space-y-4">
      ...
      {itens.length === 0 ? (
        <div className="text-center py-10 text-gray-400">
          ...
          <p className="text-sm">Nenhuma imobiliária/construtora cadastrada.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">CNPJ</th>
            ...
            <tbody>
              {itens.map((r, idx) => (
                <tr key={r.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors group${idx === itens.length - 1 ? ' border-b-0' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-gray-600">{TIPO_IMOBILIARIA_LABEL[r.tipo]}</td>
      ...
      <Dialog open={modalAberto} onOpenChange={(open: boolean) => !open && fecharModal()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editando ? 'Editar Imobiliária/Construtora' : 'Nova Imobiliária/Construtora'}</DialogTitle>
          </DialogHeader>
          {modalAberto && (
            <FormImobiliaria inicial={editando} onSalvar={handleSalvar} onCancelar={fecharModal} isPending={criar.isPending || atualizar.isPending} />
          )}
        </DialogContent>
      </Dialog>
    </div>
