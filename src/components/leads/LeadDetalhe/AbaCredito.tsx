...
import {
  ...
  ClipboardList, ExternalLink, Pencil,
} from 'lucide-react'
...
// Campos extensíveis — adicionar entradas aqui + migration para novos campos
const CAMPOS_IMOVEL = [
  ...
  // futuras: { key: 'imovel_uf', label: 'UF', tipo: 'text' },
] as const

const CAMPOS_VENDEDOR = [
  { key: 'vendedor_nome',     label: 'Nome',     tipo: 'text' as const },
  { key: 'vendedor_cpf',      label: 'CPF',      tipo: 'text' as const },
  { key: 'vendedor_telefone', label: 'Telefone', tipo: 'text' as const },
  // futuras: { key: 'vendedor_estado_civil', label: 'Estado Civil', tipo: 'text' },
  // futuras: { key: 'vendedor_banco', label: 'Banco', tipo: 'text' },
  // futuras: { key: 'vendedor_agencia', label: 'Agência/Conta', tipo: 'text' },
] as const
...
export function AbaCredito({ lead }: Props) {
  ...
  const [vendedorDialogAberto, setVendedorDialogAberto] = useState(false)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 5. Vendedor */}
      <BlocoVendedor lead={lead} onEditar={() => setVendedorDialogAberto(true)} />
      ...
      {vendedorDialogAberto && (
        <VendedorEditarDialog
          lead={lead}
          onClose={() => setVendedorDialogAberto(false)}
        />
      )}
    </div>
  )
...
function BlocoParticipantes({ lead, onCompletarPessoa, onAbrirConjugePessoa, onEditarConjuge, onDesvincularConjuge, onCriarConjuge }: {
  ...
  const rendaFormalConjuge      = lead.conjuge_renda_formal    ?? 0
  const rendaInformalConjuge    = lead.conjuge_renda_informal  ?? 0
  const totalConjuge            = rendaFormalConjuge + rendaInformalConjuge
  ...
  function salvarRendaConjuge() {
    editar.mutate({
      id: lead.id,
      conjuge_renda_formal:   parseMoeda(conjFormal),
      conjuge_renda_informal: parseMoeda(conjInformal),
    ...
    <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-1">
        ...
        <div className="ml-12 mt-2 grid grid-cols-3 gap-2">
          <RendaCampo label="Formal" valor={fmtMoeda(rendaFormalComprador || null)} />
          <RendaCampo label="Informal" valor={fmtMoeda(rendaInformalComprador || null)} />
          <RendaCampo label="Total" valor={fmtMoeda(totalComprador || null)} destaque />
        </div>
      </div>
      ...
      {casado && (
        <>
          <div className="border-t border-dashed border-gray-100 mx-4 my-2" />
              ...
              <>
                ...
                <div className="ml-12 mt-2 grid grid-cols-3 gap-2">
                  ...
                  <RendaCampoEditavel
                    ...
                    onBlur={() => { if (conjDirty) salvarRendaConjuge() }}
                  />
                  <RendaCampo label="Total" valor={fmtMoeda((parseMoeda(conjFormal) ?? 0) + (parseMoeda(conjInformal) ?? 0) || null)} destaque />
                </div>
                {conjDirty && (
                  <div className="ml-12 mt-1">
                    <button onClick={salvarRendaConjuge} className="flex items-center gap-1 text-[10px] text-[#253B29] hover:underline">
                      {editar.isPending ? <Loader2 className="h-2.5 w-2.5 animate-spin" /> : <Save className="h-2.5 w-2.5" />}
                      Salvar renda
                    </button>
                  </div>
                )}
      ...
      {rendaTotal > 0 && (
        <div className="border-t border-gray-100 bg-gray-50 px-4 py-2.5 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-bold text-[#253B29]">{fmtMoeda(rendaTotal)}</p>
          </div>
          <div className="text-right">
            ...
            {lead.renda_considerada && (
              <p className="text-[10px] text-gray-400">
                máx 30%: {fmtMoeda(lead.renda_considerada * 0.30)}
              </p>
            )}
          </div>
        </div>
...
// ── BlocoImovel ───────────────────────────────────────────────
// Para adicionar campos: incluir em CAMPOS_IMOVEL + migration
...
// ── BlocoVendedor ─────────────────────────────────────────────
// Para adicionar campos: incluir em CAMPOS_VENDEDOR + migration

function BlocoVendedor({ lead, onEditar }: { lead: Lead; onEditar: () => void }) {
  const temDados = CAMPOS_VENDEDOR.some(c => lead[c.key as keyof Lead] != null)
    ...
    return (
      <div className="bg-white border border-dashed border-gray-200 rounded-lg p-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onEditar} className="flex items-center gap-1 text-xs text-[#253B29] hover:underline font-medium">
            <Plus className="h-3 w-3" /> Informar Vendedor
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onEditar} className="text-xs text-gray-400 hover:text-[#253B29] flex items-center gap-0.5">
          <Pencil className="h-3 w-3" /> Editar
        </button>
      </div>
      <div className="grid grid-cols-2 gap-x-6 gap-y-2">
...
function VendedorEditarDialog({ lead, onClose }: { lead: Lead; onClose: () => void }) {
  const editar = useEditarLead()
  const [form, setForm] = useState<Record<string, string>>(
    Object.fromEntries(CAMPOS_VENDEDOR.map(c => [c.key, (lead[c.key as keyof Lead] as string) ?? '']))
  )

  function salvar() {
    const patch: Record<string, string | null> = {}
    for (const c of CAMPOS_VENDEDOR) patch[c.key] = form[c.key] || null
    editar.mutate({ id: lead.id, ...patch as any }, { onSuccess: onClose })
  }

  return (
    <Dialog open onOpenChange={v => { if (!v) onClose() }}>
      <DialogContent className="max-w-sm p-6">
        <h2 className="text-base font-semibold text-[#253B29] mb-4">Dados do Vendedor</h2>
