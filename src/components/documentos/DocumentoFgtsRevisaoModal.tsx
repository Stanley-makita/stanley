...
const CAMPOS_LABELS: Record<string, string> = {
  cod_empregador:  'Código do empregador',
  nro_conta_fgts:  'Número da conta FGTS',
  pis_pasep:       'NIS / PIS / PASEP',
  saldo_disponivel: 'Saldo disponível (R$)',
  data_extrato:    'Data do extrato',
}
...
export function DocumentoFgtsRevisaoModal({ documento, onClose, onConfirmado }: Props) {
  ...
  async function handleConfirmar() {
    ...
    try {
      ...
      toast.success('Dados FGTS confirmados e salvos no perfil do cliente.')
      onConfirmado()
    } catch {
        ...
        <div className="px-6 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">FGTS</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs text-gray-500">
              Verifique os dados extraídos do extrato. Ao confirmar, serão salvos no perfil do cliente.
            </p>
            <button
          ...
          {Object.entries(CAMPOS_LABELS).map(([key, label]) => (
            <div key={key} className="mb-3">
              ...
              <input
                ...
                placeholder={key === 'saldo_disponivel' ? '1234.56' : `${label}...`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-between gap-3 px-6 pb-5 pt-3 border-t border-gray-100 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={salvando}
            >
              {salvando ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Confirmar FGTS'}
            </Button>
          </div>
        </div>
      </DialogContent>
