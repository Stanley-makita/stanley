...
import { NOTA_IOF_CGI, notaTaxaCgi, BANCOS_CGI_CONFIG } from './constantes'
...
export async function gerarPDFCgiBuffer(
  ...
  const cols = [
    { label: 'Banco',         w: usableW * 0.16 },
    { label: 'Sistema',       w: usableW * 0.10 },
    { label: 'Vlr Simulado',  w: usableW * 0.16 },
    { label: '% Imóvel',      w: usableW * 0.10 },
    { label: 'Taxa a.a.',     w: usableW * 0.12 },
    { label: 'Prazo',         w: usableW * 0.10 },
    { label: 'IOF Estimado',  w: usableW * 0.13 },
    { label: 'Prestação',     w: usableW * 0.13 },
  ]
  setFill(doc, COR_VERDE)
  doc.rect(mL, y, usableW, 8, 'F')
  let cx = mL
  doc.setFontSize(6.3); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255)
  cols.forEach((col) => {
  ...
  resultado.bancos.forEach((b, idx) => {
    ...
    setFill(doc, ehMenor ? '#EEF5EE' : idx % 2 === 0 ? '#F8F8F5' : '#FFFFFF')
  ...
  // ── LIMITAÇÕES (LTV/prazo) ───────────────────────────────────────────────
  const bancosLimitados = resultado.bancos.filter((b) => b.limitadoPeloLtv || b.limitadoPeloPrazo)
  if (bancosLimitados.length > 0) {
    ...
    const textoLimitacoes = bancosLimitados.map((b) => {
      ...
      if (b.limitadoPeloPrazo) partes.push(`prazo limitado a ${b.prazoConsiderado} meses`)
